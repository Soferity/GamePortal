...
const highScoreKey = 'menja_score';
const getHighScore = () => {
   const raw = Taiizor.Storage.Local.Get(highScoreKey);
...
const setHighScore = score => {
   ...
   Taiizor.Storage.Local.Set(highScoreKey, String(score))
};
...
function renderScoreHud() {
   if (isCasualGame()) {
      ...
      scoreNode.innerText = `${String.format(livescore, state.game.score)}`;
      scoreNode.style.display = 'block';
      cubeCountNode.style.opacity = 0.65;
   }
   cubeCountNode.innerText = `${String.format(cubesmash, state.game.cubeCount)}`;
}
...
function renderMenus() {
   ...
   switch (state.menus.active) {
      ...
      case MENU_SCORE:
         ...
         if (isNewHighScore()) {
            highScoreLblNode.textContent = newscore;
         } else {
            highScoreLblNode.textContent = `${String.format(highscore, formatNumber(getHighScore()))}`;
         }
...
handleClick($('.play-normal-btn'), () => {
   Taiizor.Storage.Local.Set('menja', 'play');
...
handleClick($('.menu-btn--exit'), () => Taiizor.Redirect.Location(''));
...
handleClick($('.menu-btn--exit2'), () => Taiizor.Redirect.Location(''));
...
function setupCanvases() {
   Taiizor.Style.Add("body", "background-color", "#000");
   Taiizor.Style.Add("body", "background-image", "radial-gradient(ellipse at top, #335476 0.0%, #31506e 11.1%, #304b67 22.2%, #2f4760 33.3%, #2d4359 44.4%, #2c3f51 55.6%, #2a3a4a 66.7%, #293643 77.8%, #28323d 88.9%, #262e36 100.0%)");
...
function SetLocalization(NewScore, HighScore, LiveScore, CubeSmash) {
    newscore = NewScore;
    highscore = HighScore;
    livescore = LiveScore;
    cubesmash = CubeSmash;
